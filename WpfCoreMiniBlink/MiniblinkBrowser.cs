...
namespace MiniblinkCore
{
    public class MiniblinkBrowser : Border, IMiniblink, INotifyPropertyChanged
    {
        ...
        /// 页面加载完成
        /// 
        /// 注意：由于现在网页多采用异步加载模式，Document数据结构生成完成 ≠ 网页加载完成，每个异步执行的加载动作完成时都可能会触发一次本回调，
        /// 注意是可能，不是一定，另外页面中如有多个frame，每个frame加载完成时都会触发一次本回调，实践中可以采用wkeOnLoadingFinish接口，
        /// wkeOnTitleChanged接口，wkeOnURLChanged接口，以及检查页面中某个元素（如网站logo等）是否存在等方法来综合判断页面加载是否完成。
        /// </summary>
        ...
        private EventHandler<LoadingFinishEventArgs> _loadingFinishHandler = null;
        /// <summary>
        /// 载入完成(和DocumentReady一样)
        /// </summary>
        public event EventHandler<LoadingFinishEventArgs> LoadingFinish
        {
            add
            {
                if (_loadingFinishHandler == null)
                {
                    MBApi.wkeOnLoadingFinish(MiniblinkHandle, new wkeLoadingFinishCallback((webview,param,url,result, failedReason)=> {
                        _loadingFinishHandler?.Invoke(this, new LoadingFinishEventArgs(webview, url, result, failedReason));
                    }), IntPtr.Zero);
                }
                _loadingFinishHandler += value;
            }
            remove
            {
                _loadingFinishHandler -= value;
                if (_loadingFinishHandler == null)
                {
                    MBApi.wkeOnLoadingFinish(MiniblinkHandle, null, IntPtr.Zero);
                }
            }
        }
        ...
        public void ShowDevTools()
        {
            ...
            var dir = Path.Combine(System.Environment.CurrentDirectory, "front_end");
            if (Directory.Exists(dir) == false)
            {
                ...
                {
                    using (var zip = Ionic.Zip.ZipFile.Read(sm))
                    {
        ...
        public MiniblinkBrowser()
        {
            ...
            if (!Utils.IsDesignMode())
            {
                ...
                MBApi.wkeSetNavigationToNewWindowEnable(MiniblinkHandle, false);
